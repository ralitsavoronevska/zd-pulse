...
// refresh so we don't burn through the rotating refresh token. Cleared via
// `queueMicrotask` so the slot is freed AFTER all awaiters of
// `_refreshPromise` have resumed (see comment at the assignment site for
// why direct `.finally` nulling races).
...
api.interceptors.response.use(
    ...
    async (error) => {
        ...
        try {
            if (!_refreshPromise) {
                // The `.finally` callback runs in microtask order BEFORE
                // sibling awaiters (their `await _refreshPromise` handlers
                // were registered later). Nulling synchronously inside
                // `.finally` therefore frees the slot before A and B resume —
                // and a 401 fired during their resumption sees `null` and
                // starts a SECOND refresh, double-spending the rotating
                // refresh token.
                //
                // `queueMicrotask` schedules the null-out at the END of the
                // current microtask cycle, after every queued awaiter has run.
