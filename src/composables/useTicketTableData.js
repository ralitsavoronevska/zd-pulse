...
import { maskCustomerEmail } from '@/utils/stringUtils';
...
export function useTicketTableData(filterState, dataTableRef) {
    ...
    const maskCustomerEmailRow = (ticket) => ({
        ...ticket,
        customer_email: maskCustomerEmail(ticket.customer_email)
    });
    ...
    const tableData = computed(() => {
        if (USE_MOCKED) {
            ...
            return shouldMaskCustomerEmails.value ? rows.map(maskCustomerEmailRow) : rows;
        }
    ...
    const exportRows = USE_MOCKED ? computed(() => (shouldMaskCustomerEmails.value ? mockedFilteredTickets.value.map(maskCustomerEmailRow) : mockedFilteredTickets.value)) : null;
