...
import { buildFacetedFilterOptionsParams, buildNarrowedFilterOptionsParams, buildStatsParams, buildTopicChartParams, buildVipCsatParams, fetchFilterOptions, fetchTicketStats, fetchTopicChartData, fetchVipCsatData } from '@/services/ticketApi';

// Multi/single-select fields that get a per-field facet response when active.
// Each key is the API param name returned in /api/ticket-filter-options/;
// the value is the corresponding key in the local `filters` object.
const FACETED_FIELDS = ['brand', 'topic', 'vip_level', 'agent_email', 'customer_email', 'chat_tags', 'csat_score', 'sentiment'];
const FACETED_FILTER_KEY = {
    brand: 'brand',
    topic: 'topic',
    vip_level: 'vip_level',
...
function isFieldActive(filters, field) {
    const v = filters?.[FACETED_FILTER_KEY[field]];
...
export const useTableStore = defineStore('table', () => {
    ...
    // ════════════════════════════════════════════════════════════════════
    // Per-active-field faceted responses. Map from API field name (e.g.
    // 'csat_score') to the array of distinct values returned when that field's
    // own filter was excluded from the request. Stale entries from previously
    // active fields are tolerated — `useTicketTableData.smartOptions` only
    // reads the entries whose field is currently active.
    const facetedFilterOptions = ref({});
    ...
    // Per-field generation counters so a stale faceted response can never
    // overwrite a fresher one. Each key is the API param name.
    const facetGenerations = Object.fromEntries(FACETED_FIELDS.map((f) => [f, 0]));
    ...
    /**
     * FACETED options for one active field — date-range + every attribute
     * filter EXCEPT the named field's own. Lets the field's own dropdown
     * show what other values remain plausible given the OTHER filters,
     * instead of self-narrowing to the single selected value.
     * Case-insensitive fields are lowercased + deduped via `normalizeFilterOptions`.
     */
    async function fetchFacetedFilterOption(filters, field) {
        const generation = ++facetGenerations[field];
        try {
            const data = await fetchFilterOptions(buildFacetedFilterOptionsParams(filters, field));
            if (generation !== facetGenerations[field]) return;
            const normalized = normalizeFilterOptions(data);
            facetedFilterOptions.value = { ...facetedFilterOptions.value, [field]: normalized?.[field] ?? [] };
        } catch (err) {
            if (generation === facetGenerations[field]) {
                logger.error(`fetchFacetedFilterOption(${field}) failed:`, err?.message || err);
            }
        }
    }

    /** Fetches a faceted response for every currently-active multi/single-select field. */
    async function fetchAllActiveFacets(filters) {
        const activeFields = FACETED_FIELDS.filter((f) => isFieldActive(filters, f));
        if (!activeFields.length) return;
        await Promise.allSettled(activeFields.map((f) => fetchFacetedFilterOption(filters, f)));
    }
    ...
    async function fetchFilterOptionsOnly(filters) {
        ...
        try {
            await Promise.allSettled([fetchNarrowedFilterOptions(filters), fetchAllActiveFacets(filters)]);
        } finally {
    ...
    async function fetchCoreAggregations(filters) {
        incrementAggregationLoading();
        try {
            await Promise.allSettled([fetchNarrowedFilterOptions(filters), fetchAllActiveFacets(filters), fetchStats(filters)]);
        } finally {
            decrementAggregationLoading();
        }
    }
