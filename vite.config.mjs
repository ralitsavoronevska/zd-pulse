...
export default defineConfig({
    ...
    base: '/zd-pulse/', // Uncomment and set this if deploying to a subdirectory (e.g., GitHub Pages)

    resolve: {
        alias: {
            '@': fileURLToPath(new URL('./src', import.meta.url)),
            '@rootfirebase': fileURLToPath(new URL('./firebase', import.meta.url))
        }
    },
    ...
    plugins: [
        ...
        // Replace mock JSON with empty array in production ONLY when NOT using mocked data
        // (prevents 973 kB dead chunk in dist/ when API mode is enabled)
        {
            name: 'exclude-mock-data',
            enforce: 'pre',
            resolveId(source) {
                const useMocked = env.VITE_USE_MOCKED_DATA === 'true';
        ...
        },
        {
            ...
            generateBundle(_, bundle) {
                ...
                // Rewrite CSS url() references from hashed assets to /zd-pulse/fonts/primeicons/
                for (const key of Object.keys(bundle)) {
